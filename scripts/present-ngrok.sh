...
if curl -s -m 3 localhost:4040/api/tunnels 2>/dev/null | grep -q "$DOMAIN"; then
  ...
  if curl -s -m 2 localhost:4040/api/tunnels 2>/dev/null | grep -q "$DOMAIN"; then
    ...
    warn "tunnel NOT up (no internet? see $NGROK_LOG) — everything else still works on localhost; re-run when online"
  fi
fi

step "5/6 companion :7070 (QR must encode the PUBLIC url)"
if curl -sf -m 3 http://127.0.0.1:7070/qr 2>/dev/null | grep -q "$DOMAIN"; then
  ok "companion up and QR encodes $DOMAIN"
else
  # up-but-wrong-URL (started without COMPANION_URL) also lands here: restart it
  pkill -f 'companion/app.py' 2>/dev/null; sleep 1
  nohup env COMPANION_URL="$URL" EXPOSE_CONNECT=1 make companion >"$COMPANION_LOG" 2>&1 &
